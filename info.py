...
API_ID = int(environ.get('API_ID', '8962471'))
API_HASH = environ.get('API_HASH', '27a3ded1ca63bb3d41d4b00a9be23fef')
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '1833450791').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1001669430720').split()] #Channel id for auto indexing ( make sure bot is admin )
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '').split()]
auth_channel = environ.get('AUTH_CHANNEL', '-1001507970860') #Channel / Group Id for force sub ( make sure bot is admin )
auth_grp = environ.get('AUTH_GROUP')
AUTH_CHANNEL = int(auth_channel) if auth_channel and id_pattern.search(auth_channel) else None
AUTH_GROUPS = [int(ch) for ch in auth_grp.split()] if auth_grp else None
support_chat_id = environ.get('SUPPORT_CHAT_ID', '-1001779842957') # support group id ( make sure bot is admin )
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1001779842957') # request channel id ( make sure bot is admin )
...
DATABASE_URI = environ.get('DATABASE_URI', "mongodb+srv://backupkrnewalamovieproviderbot:mEtG827tOkOccTzj@cluster0.kylwqkk.mongodb.net/?retryWrites=true&w=majority&appName=Cluster0")
...
GRP_LNK = environ.get('GRP_LNK', 'https://t.me/StreamTVIndex')
CHNL_LNK = environ.get('CHNL_LNK', 'https://t.me/StreamTVIndex')
...
LOG_CHANNEL = int(environ.get('LOG_CHANNEL', '-1001988724074')) #Log channel id ( make sure bot is admin )
SUPPORT_CHAT = environ.get('SUPPORT_CHAT', 'https://t.me/StreamTVIndex') #Support group link ( make sure bot is admin )
...
FILE_STORE_CHANNEL = [int(ch) for ch in (environ.get('FILE_STORE_CHANNEL', '-1002159220749')).split()]
